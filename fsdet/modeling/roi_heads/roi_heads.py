...
class LKA(nn.Module):
    def __init__(self, dim, dilation):
        ...
        self.conv_spatial = nn.Conv2d(dim, dim, 7, stride=1, padding=9, groups=dim, dilation=dilation)
...
class EFM(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj_1(x)
        x = self.activation(x)
        x = torch.cat(self.lka2(x), self.lka3(x))
        x = self.proj_2(x)
        x = x + shorcut
    ...
    def forward(self, images, features, proposals, targets=None, alg_iter=0):
        ...
        if self.training:
            ...
            losses = self._forward_box(features_list, proposals,
                                       alg_iter)  # get losses from fast_rcnn.py::FastRCNNOutputs
...
class TernaryHiercContraROIHeads(ROIHeads):
    ...
    def forward(self, images, features, proposals, targets=None, alg_iter=0, for_calculate_loss=None,
                batched_inputs=None):
        """
        See :class:`ROIHeads.forward`.
            proposals (List[Instance]): fields=[proposal_boxes, objectness_logits]
        ...
        if self.training:
            ...
            # {'loss_cls':, 'loss_box_reg':}
            losses = self._forward_box(features_list, proposals,
                                       alg_iter)  # get losses from fast_rcnn.py::FastRCNNOutputs (sub classification)
            rpn_losses = self._forward_rpn_box(features_list, proposals, alg_iter, for_calculate_loss, batched_inputs,
                                               images)
            del images
    ...
    def _forward_rpn_box(self, features, proposals, alg_iter=0, for_calculate_loss=None, batched_inputs=None,
                         images=None):
        """
        Forward logic of the box prediction branch.
        ...
        for i in range(len(test_pred_class_hierc)):
            test_pred_class_hierc_i = torch.eq(test_pred_class_hierc[i] >= 0,
                                               test_pred_class_hierc[i] < self.num_classes)
        ...
        for_calculate_loss["gt_objectness_logits"] = torch.split(for_calculate_loss["gt_objectness_logits"],
                                                                 for_calculate_loss["num_anchors_per_map"], dim=1)
        # Concat from all feature maps
        for_calculate_loss["gt_objectness_logits"] = cat(
            [x.flatten() for x in for_calculate_loss["gt_objectness_logits"]], dim=0)
        ...
        if self.is_rpn_contra:
            ...
            loss_contrast = self.contrast_loss_weight * self.supervised_contrastive_loss(proposal_features_contrast,
                                                                                         contra_gt_class, contra_iou)
...
class NovelRoiHeads(StandardROIHeads):
    ...
    def _forward_box(self, features, proposals):
        ...
        if self.output_layer_name == 'FastRCNNDoubleHeadCosMarginLayers':
            ...
            pred_class_logits, pred_proposal_deltas = self.box_predictor(box_loc_feat, box_cls_feat, gt_classes,
                                                                         self.training)
        else:
            pred_class_logits, pred_proposal_deltas = self.box_predictor(box_loc_feat, box_cls_feat)
