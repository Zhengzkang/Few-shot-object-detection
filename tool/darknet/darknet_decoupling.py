...
import numpy as np
from core.region_loss import RegionLossV3
from core.cfg import *
# from dynamic_conv import DynamicConv2d
from core.dynamic_conv import dynamic_conv2d
from core.pooling import GlobalMaxPool2d
from core.pooling import GlobalAvgPool2d
from core.pooling import Split
import pdb
...
class Darknet(nn.Module):
    ...
    def meta_forward(self, metax, mask):
        ...
        self.loss = None
        outputs = dict()
        for learnet_block in self.learnet_blocks:
            ind = ind + 1
            #if ind > 0:
            #    return metax

            if learnet_block['type'] == 'learnet':
                continue
            elif learnet_block['type'] == 'convolutional' or \
                 learnet_block['type'] == 'maxpool' or \
                 learnet_block['type'] == 'avgpool' :
                metax = self.learnet_models[ind](metax)
                outputs[ind] = metax
            elif learnet_block['type'] == 'globalavg' or \
                 learnet_block['type'] == 'globalmax':
                metax = self.learnet_models[ind](metax)
            elif learnet_block['type'] == 'connected':
                dynamic_weights.append(metax)
                metax = metax.squeeze(3)
                metax = metax.squeeze(2)
                metax = self.learnet_models[ind](metax)
                outputs[ind] = metax
    ...
    def forward(self, x, metax, mask, ids=None):
        # pdb.set_trace()
        dynamic_weights, decoupling_prototype = self.meta_forward(metax, mask)
        x = self.detect_forward(x, dynamic_weights)
        return x,decoupling_prototype
